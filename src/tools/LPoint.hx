...
class LPoint {
  ...
  inline function get_globalX():Float {
    if (Game.ME == null || Game.ME.destroyed) return -1; else
  ...
  public inline function distCase(?e:Entity, ?pt:LPoint, ?cx = 0, ?cy = 0,
      ?xr = 0.5, ?yr = 0.5) {
    if (e != null) return M.dist(this.cx + this.xr, this.cy + this.yr,
      e.cx + e.xr,
      e.cy + e.yr); else if (pt != null) return M.dist(this.cx + this.xr,
      this.cy + this.yr, pt.cx + pt.xr, pt.cy + pt.yr); else
  ...
  public inline function distPx(?e:Entity, ?pt:LPoint, ?x = 0., ?y = 0.) {
    if (e != null) return M.dist(levelX, levelY, e.footX,
      e.footY); else if (pt != null) return M.dist(levelX, levelY, pt.levelX,
      pt.levelY); else
  ...
  public inline function angTo(?e:Entity, ?pt:LPoint, ?x = 0., ?y = 0.) {
    if (e != null) return Math.atan2((e.cy + e.yr) - cyf,
      (e.cx
        + e.xr) - cxf); else if (pt != null) return Math.atan2(pt.cyf - cyf,
      pt.cxf - cxf); else
